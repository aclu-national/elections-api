// Example response from https://elections.api.aclu.org/v2/pip?lat=40.7023587&lng=-74.0124621

{
  calendar_url: "https://elections.api.aclu.org/v2/calendar?state=ny&format=ics",
  congress: {
    district: {
      aclu_id: "aclu/elections-api/congress_district:11084",
      area: 59669583.267117,
      at_large: false,
      district_num: 10,
      end_date: "2019-01-03",
      end_session: 115,
      geoid: "3610",
      name: "NY Congressional District 10",
      non_voting: false,
      ocd_id: "ocd-division/country:us/state:ny/cd:10",
      start_date: "2017-01-03",
      start_session: 115,
      state: "ny",
      state_full: "New York"
    },
    legislators: [{
        bio: {
          birthday: "1950-11-23",
          gender: "M"
        },
        contact: {
          address: "322 Hart Senate Office Building Washington DC 20510",
          contact_form: "https://www.schumer.senate.gov/contact/email-chuck",
          fax: "202-228-3027",
          office: "322 Hart Senate Office Building",
          phone: "202-224-6542",
          url: "https://www.schumer.senate.gov"
        },
        id: {
          aclu_id: "aclu/elections-api/congress_legislator:854",
          ballotpedia: "Chuck Schumer",
          bioguide: "S000148",
          cspan: "5929",
          fec: "S8NY00082,H0NY16010",
          google_entity_id: "kg:/m/01w74d",
          govtrack: "300087",
          house_history: "21322",
          icpsr: "14858",
          lis: "S270",
          maplight: "606",
          opensecrets: "N00001093",
          thomas: "01036",
          votesmart: "26976",
          wikidata: "Q380900",
          wikipedia: "Chuck Schumer"
        },
        name: {
          first_name: "Charles",
          full_name: "Charles "
          Chuck " E. Schumer",
          last_name: "Schumer",
          nickname: "Chuck"
        },
        photo: "https://elections.api.aclu.org/congress_photos/S000148.jpg",
        score_sessions: [
          115,
          116
        ],
        sessions: [{
            display_name: "Leader Charles "
            Chuck " Schumer",
            running_for_president: false,
            running_in_2018: false,
            session: 115,
            total_score: "70%",
            votes_agreed: "19",
            votes_total: "27"
          },
          {
            display_name: "Sen. Chuck Schumer",
            running_for_president: false,
            session: 116,
            total_score: "78%",
            votes_agreed: "7",
            votes_total: "9"
          }
        ],
        social: {
          facebook: "senschumer",
          twitter: "SenSchumer",
          twitter_id: "17494010",
          youtube: "SenatorSchumer",
          youtube_id: "UC-ABttxh8uQv_10qmwGaidw"
        },
        term: {
          class: 3,
          end_date: "2023-01-03",
          is_delegate: false,
          office: "us_senator",
          party: "Democrat",
          start_date: "2017-01-03",
          state: "ny",
          state_full: "New York",
          state_rank: "senior",
          term_ended_early: false,
          term_started_late: false
        },
        url_slug: "ny-charles-schumer"
      },
      {
        bio: {
          birthday: "1966-12-09",
          gender: "F"
        },
        contact: {
          address: "478 Russell Senate Office Building Washington DC 20510",
          contact_form: "https://www.gillibrand.senate.gov/contact/email-me",
          office: "478 Russell Senate Office Building",
          phone: "202-224-4451",
          rss_url: "http://www.gillibrand.senate.gov/rss/",
          url: "https://www.gillibrand.senate.gov"
        },
        id: {
          aclu_id: "aclu/elections-api/congress_legislator:869",
          ballotpedia: "Kirsten Gillibrand",
          bioguide: "G000555",
          cspan: "1022862",
          fec: "H6NY20167,S0NY00410",
          google_entity_id: "kg:/m/0gnfc4",
          govtrack: "412223",
          house_history: "14269",
          icpsr: "20735",
          lis: "S331",
          maplight: "708",
          opensecrets: "N00027658",
          thomas: "01866",
          votesmart: "65147",
          wikidata: "Q22222",
          wikipedia: "Kirsten Gillibrand"
        },
        name: {
          first_name: "Kirsten",
          full_name: "Kirsten E. Gillibrand",
          last_name: "Gillibrand",
          nickname: null
        },
        photo: "https://elections.api.aclu.org/congress_photos/G000555.jpg",
        score_sessions: [
          115,
          116
        ],
        sessions: [{
            display_name: "Sen. Kirsten Gillibrand",
            running_for_president: false,
            running_in_2018: true,
            session: 115,
            total_score: "85%",
            votes_agreed: "23",
            votes_total: "27"
          },
          {
            display_name: "Sen. Kirsten Gillibrand",
            running_for_president: false,
            session: 116,
            total_score: "100%",
            votes_agreed: "9",
            votes_total: "9"
          }
        ],
        social: {
          facebook: "SenKirstenGillibrand",
          twitter: "GillibrandNY",
          twitter_id: "899978622416695297",
          youtube: "KirstenEGillibrand",
          youtube_id: "UCVEloQogVsmnkd5vxJInmYg"
        },
        term: {
          class: 1,
          end_date: "2025-01-03",
          is_delegate: false,
          office: "us_senator",
          party: "Democrat",
          start_date: "2019-01-03",
          state: "ny",
          state_full: "New York",
          state_rank: "junior",
          term_ended_early: false,
          term_started_late: false
        },
        url_slug: "ny-kirsten-gillibrand"
      },
      {
        bio: {
          birthday: "1947-06-13",
          gender: "M"
        },
        contact: {
          address: "2132 Rayburn House Office Building; Washington DC 20515-3210",
          office: "2132 Rayburn House Office Building",
          phone: "202-225-5635",
          rss_url: "http://nadler.house.gov/rss.xml",
          url: "https://nadler.house.gov"
        },
        id: {
          aclu_id: "aclu/elections-api/congress_legislator:863",
          bioguide: "N000002",
          cspan: "26159",
          fec: "H2NY17071",
          google_entity_id: "kg:/m/03tk11",
          govtrack: "400289",
          house_history: "18837",
          icpsr: "29377",
          maplight: "385",
          opensecrets: "N00000939",
          thomas: "00850",
          votesmart: "26980",
          wikidata: "Q505598",
          wikipedia: "Jerrold Nadler"
        },
        name: {
          first_name: "Jerrold",
          full_name: "Jerrold Nadler",
          last_name: "Nadler",
          nickname: null
        },
        photo: "https://elections.api.aclu.org/congress_photos/N000002.jpg",
        score_sessions: [
          115,
          116
        ],
        sessions: [{
            display_name: "Rep. Jerry Nadler",
            running_for_president: false,
            running_in_2018: true,
            session: 115,
            total_score: "87%",
            votes_agreed: "34",
            votes_total: "39"
          },
          {
            display_name: "Rep. Jerry Nadler",
            running_for_president: false,
            session: 116,
            total_score: "83%",
            votes_agreed: "24",
            votes_total: "29"
          }
        ],
        social: {
          facebook: "CongressmanNadler",
          twitter: "RepJerryNadler",
          twitter_id: "40302336",
          youtube: "congressmannadler",
          youtube_id: "UCQKj0qU1e7o2f5ftSh-c9aQ"
        },
        term: {
          district_num: 10,
          end_date: "2021-01-03",
          is_delegate: false,
          office: "us_representative",
          party: "Democrat",
          start_date: "2019-01-03",
          state: "ny",
          state_full: "New York",
          term_ended_early: false,
          term_started_late: false
        },
        url_slug: "ny-jerrold-nadler"
      }
    ],
    next_district: {
      aclu_id: "aclu/elections-api/congress_district:11084",
      area: 59669583.267117,
      at_large: false,
      district_num: 10,
      end_date: "2019-01-03",
      end_session: 115,
      geoid: "3610",
      name: "NY Congressional District 10",
      non_voting: false,
      ocd_id: "ocd-division/country:us/state:ny/cd:10",
      start_date: "2017-01-03",
      start_session: 115,
      state: "ny",
      state_full: "New York"
    }
  },
  county: {
    aclu_id: "aclu/elections-api/county:9592",
    area_land: 58681329,
    area_water: 28550878,
    geoid: "36061",
    name: "New York",
    ocd_id: "ocd-division/country:us/state:ny/county:new_york",
    state: "ny"
  },
  elections: {
    ballots: [{
        date: "2018-06-26",
        initiatives: [],
        offices: {
          county: [],
          federal: [{
              blurb: {
                details: [
                  "Pass the Dream Act",
                  "Pass the Equality Act",
                  "End mandatory minimums",
                  "Protect access to birth control"
                ],
                details_title: "Vote for a U.S. Senator who will vote to",
                name: "U.S. Senator",
                summary: "Your Senator serves an important role in passing the laws of the land, acting as a check on the President, confirming presidential nominees and federal judges, and defend civil rights and civil liberties for everyone, even when it’s hard.",
                title: "What does your U.S. Senator do?"
              },
              ocd_id: "ocd-division/country:us/state:ny",
              office: "us_senator",
              races: [{
                name: "U.S. Senate New York"
              }]
            },
            {
              blurb: {
                details: [
                  "Pass the Dream Act",
                  "Restore Net Neutrality",
                  "Restore the Voting Rights Act",
                  "Pass the Equality Act",
                  "Pass the EACH Woman Act"
                ],
                details_title: "Vote for a U.S. House Representative who will vote to",
                name: "U.S. House Representative",
                summary: "Your U.S. House Representative represents their constituents in your district and also serves an important role in passing the laws of the land and in acting as a check on the President, and should put the civil rights and civil liberties of their constituents above all else.",
                title: "What does your U.S. House Representative do?"
              },
              ocd_id: "ocd-division/country:us/state:ny/cd:10",
              office: "us_representative",
              races: [{
                name: "U.S. House New York District 10"
              }]
            }
          ],
          state: []
        },
        type: "primary"
      },
      {
        date: "2018-09-13",
        initiatives: [],
        offices: {
          county: [],
          federal: [],
          state: [{
              blurb: {
                details: [
                  "Protect reproductive freedom",
                  "Expand when and how people can register to vote",
                  "End civil asset forfeiture",
                  "Fight for anti-discrimination protections for LGBT people",
                  "Reduce the number of imprisoned Americans"
                ],
                details_title: "Vote for a governor who will",
                name: "Governor",
                summary: "The Governor serves as the head of your state with the power to pass executive orders and veto bills that pass the state legislature. The governor also has pardon power, controls the state prison system, and has the power to appoint, hire and fire, and direct the work of thousands of state employees to preserve civil rights and liberties, and should be held accountable for their work.",
                title: "What does your Governor do?"
              },
              ocd_id: "ocd-division/country:us/state:ny",
              office: "governor",
              races: [{
                name: "Governor of New York"
              }]
            },
            {
              blurb: {
                details: [
                  "Uphold the rule of law",
                  "Defend and expand your civil rights and civil liberties",
                  "Stand up to the President"
                ],
                details_title: "Vote for an attorney general who will",
                name: "Attorney General",
                summary: "The Attorney General is your state’s chief lawyer and legal advisor tasked with upholding your state’s constitution.",
                title: "What does your Attorney General do?"
              },
              ocd_id: "ocd-division/country:us/state:ny",
              office: "attorney_general",
              races: [{
                name: "Attorney General of New York"
              }]
            },
            {
              blurb: {
                details: [
                  "Protect LGBT people from discrimination",
                  "End cash bail",
                  "Prevent voter suppression schemes",
                  "Protect reproductive freedom"
                ],
                details_title: "Vote for state representatives who will vote to",
                name: "State Senator",
                summary: "As members of the state senate, they introduce and vote on bills, oversee the state budget, provide oversight to the governor, and often oversee the drawing of legislative districts.",
                title: "What do your state senators do?"
              },
              ocd_id: "ocd-division/country:us/state:ny/sldu:26",
              office: "state_senator",
              races: [{
                name: "New York State Senate District 26"
              }]
            },
            {
              blurb: {
                alt_names: {
                  Assembly: "House"
                },
                details: [
                  "Protect LGBT people from discrimination",
                  "End cash bail",
                  "Prevent voter suppression schemes",
                  "Protect reproductive freedom"
                ],
                details_title: "Vote for state representatives who will vote to",
                name: "State Assembly Representative",
                summary: "As members of the state legislature, they introduce and vote on bills, oversee the state budget, provide oversight to the governor, and often oversee the drawing of legislative districts.",
                title: "What do your state representatives do?"
              },
              ocd_id: "ocd-division/country:us/state:ny/sldl:65",
              office: "state_representative",
              races: [{
                name: "New York State Assembly District 65"
              }]
            },
            {
              ocd_id: "ocd-division/country:us/state:ny",
              office: "lieutenant_governor",
              races: [{
                name: "Lieutenant Governor of New York"
              }]
            },
            {
              ocd_id: "ocd-division/country:us/state:ny",
              office: "state_comptroller",
              races: [{
                name: "New York Comptroller"
              }]
            }
          ]
        },
        type: "primary"
      },
      {
        date: "2018-11-06",
        initiatives: [],
        offices: {
          county: [],
          federal: [{
              blurb: {
                details: [
                  "Pass the Dream Act",
                  "Pass the Equality Act",
                  "End mandatory minimums",
                  "Protect access to birth control"
                ],
                details_title: "Vote for a U.S. Senator who will vote to",
                name: "U.S. Senator",
                summary: "Your Senator serves an important role in passing the laws of the land, acting as a check on the President, confirming presidential nominees and federal judges, and defend civil rights and civil liberties for everyone, even when it’s hard.",
                title: "What does your U.S. Senator do?"
              },
              ocd_id: "ocd-division/country:us/state:ny",
              office: "us_senator",
              races: [{
                candidates: [{
                    ballotpedia_url: "https://ballotpedia.org/Kirsten_Gillibrand",
                    first_name: "Kirsten",
                    last_name: "Gillibrand",
                    name: "Kirsten Gillibrand",
                    party: "Democratic Party"
                  },
                  {
                    ballotpedia_url: "https://ballotpedia.org/Chele_Farley",
                    first_name: "Chele",
                    last_name: "Farley",
                    name: "Chele Farley",
                    party: "Republican Party"
                  }
                ],
                name: "U.S. Senate New York"
              }]
            },
            {
              blurb: {
                details: [
                  "Pass the Dream Act",
                  "Restore Net Neutrality",
                  "Restore the Voting Rights Act",
                  "Pass the Equality Act",
                  "Pass the EACH Woman Act"
                ],
                details_title: "Vote for a U.S. House Representative who will vote to",
                name: "U.S. House Representative",
                summary: "Your U.S. House Representative represents their constituents in your district and also serves an important role in passing the laws of the land and in acting as a check on the President, and should put the civil rights and civil liberties of their constituents above all else.",
                title: "What does your U.S. House Representative do?"
              },
              ocd_id: "ocd-division/country:us/state:ny/cd:10",
              office: "us_representative",
              races: [{
                candidates: [{
                    ballotpedia_url: "https://ballotpedia.org/Jerrold_Nadler",
                    first_name: "Jerrold",
                    last_name: "Nadler",
                    name: "Jerrold Nadler",
                    party: "Democratic Party"
                  },
                  {
                    ballotpedia_url: "https://ballotpedia.org/Naomi_Levin",
                    first_name: "Naomi",
                    last_name: "Levin",
                    name: "Naomi Levin",
                    party: "Republican Party"
                  }
                ],
                name: "U.S. House New York District 10"
              }]
            }
          ],
          state: [{
              blurb: {
                details: [
                  "Protect reproductive freedom",
                  "Expand when and how people can register to vote",
                  "End civil asset forfeiture",
                  "Fight for anti-discrimination protections for LGBT people",
                  "Reduce the number of imprisoned Americans"
                ],
                details_title: "Vote for a governor who will",
                name: "Governor",
                summary: "The Governor serves as the head of your state with the power to pass executive orders and veto bills that pass the state legislature. The governor also has pardon power, controls the state prison system, and has the power to appoint, hire and fire, and direct the work of thousands of state employees to preserve civil rights and liberties, and should be held accountable for their work.",
                title: "What does your Governor do?"
              },
              ocd_id: "ocd-division/country:us/state:ny",
              office: "governor",
              races: [{
                name: "Governor of New York"
              }]
            },
            {
              blurb: {
                details: [
                  "Uphold the rule of law",
                  "Defend and expand your civil rights and civil liberties",
                  "Stand up to the President"
                ],
                details_title: "Vote for an attorney general who will",
                name: "Attorney General",
                summary: "The Attorney General is your state’s chief lawyer and legal advisor tasked with upholding your state’s constitution.",
                title: "What does your Attorney General do?"
              },
              ocd_id: "ocd-division/country:us/state:ny",
              office: "attorney_general",
              races: [{
                name: "Attorney General of New York"
              }]
            },
            {
              blurb: {
                details: [
                  "Protect LGBT people from discrimination",
                  "End cash bail",
                  "Prevent voter suppression schemes",
                  "Protect reproductive freedom"
                ],
                details_title: "Vote for state representatives who will vote to",
                name: "State Senator",
                summary: "As members of the state senate, they introduce and vote on bills, oversee the state budget, provide oversight to the governor, and often oversee the drawing of legislative districts.",
                title: "What do your state senators do?"
              },
              ocd_id: "ocd-division/country:us/state:ny/sldu:26",
              office: "state_senator",
              races: [{
                name: "New York State Senate District 26"
              }]
            },
            {
              blurb: {
                alt_names: {
                  Assembly: "House"
                },
                details: [
                  "Protect LGBT people from discrimination",
                  "End cash bail",
                  "Prevent voter suppression schemes",
                  "Protect reproductive freedom"
                ],
                details_title: "Vote for state representatives who will vote to",
                name: "State Assembly Representative",
                summary: "As members of the state legislature, they introduce and vote on bills, oversee the state budget, provide oversight to the governor, and often oversee the drawing of legislative districts.",
                title: "What do your state representatives do?"
              },
              ocd_id: "ocd-division/country:us/state:ny/sldl:65",
              office: "state_representative",
              races: [{
                name: "New York State Assembly District 65"
              }]
            },
            {
              ocd_id: "ocd-division/country:us/state:ny",
              office: "lieutenant_governor",
              races: [{
                name: "Lieutenant Governor of New York"
              }]
            },
            {
              ocd_id: "ocd-division/country:us/state:ny",
              office: "state_comptroller",
              races: [{
                  name: "New York Comptroller"
                },
                {
                  name: "New York Comptroller"
                }
              ]
            }
          ]
        },
        type: "general"
      }
    ],
    calendar: [{
        dates: {
          date: "2018-11-06",
          election_date: "2018-09-13",
          registration_deadline: "2018-08-19",
          vbm_end: "2018-09-12",
          vbm_start: "2018-09-06"
        },
        type: "primary"
      },
      {
        dates: {
          election_date: "2018-11-06",
          registration_deadline: "2018-10-12",
          vbm_end: "2018-11-05",
          vbm_end_postmarkedby: "2018-11-05",
          vbm_end_receivedby: "2018-11-13",
          vbm_start: "2018-10-30"
        },
        type: "general"
      }
    ],
    info: {
      early_voting: "no",
      same_day: "no",
      state: "ny",
      vote_by_mail: "excuse-only",
      voter_id_req: "none"
    },
    links: {
      check_reg_url: "http://voterlookup.elections.state.ny.us/",
      election_info_url: null,
      online_reg_url: "http://www.elections.ny.gov/NYSBOE/download/voting/voteform_enterable.pdf",
      polling_place_url: "http://voterlookup.elections.state.ny.us/",
      same_day_url: null,
      vbm_req_url: null,
      voter_id_url: null
    }
  },
  google_civic_info: [],
  id: "38-9592-5051-5162-11084-11084",
  ok: true,
  state: {
    aclu_id: "aclu/elections-api/state:38",
    area_land: 122048992746,
    area_water: 19247151848,
    geoid: "36",
    name: "New York",
    ocd_id: "ocd-division/country:us/state:ny",
    state: "ny"
  },
  state_leg: [{
      aclu_id: "aclu/elections-api/state_leg:5051",
      area_land: 5471178,
      area_water: 6833949,
      chamber: "lower",
      district_num: "065",
      geoid: "36065",
      name: "Assembly District 65",
      ocd_id: "ocd-division/country:us/state:ny/sldl:65",
      state: "ny"
    },
    {
      aclu_id: "aclu/elections-api/state_leg:5162",
      area_land: 16356699,
      area_water: 8726262,
      chamber: "upper",
      district_num: "026",
      geoid: "36026",
      name: "State Senate District 26",
      ocd_id: "ocd-division/country:us/state:ny/sldu:26",
      state: "ny"
    }
  ]
}
